...
pub unsafe extern "C" fn game_attack11(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        macros::ATTACK(
            agent, 0, 0, Hash40::new("top"), 4.0, 361, 35, 0, 25, 4.0, 0.0, 7.0, 6.0,
            Some(0.0), Some(7.0), Some(10.0), 1.0, 1.0, *ATTACK_SETOFF_KIND_ON,
            *ATTACK_LR_CHECK_F, false, 0, 0.0, 0, false, false, false, false, true,
            *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL,
            *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_normal"),
            *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_PUNCH, *ATTACK_REGION_PUNCH,
        );
    }
...
pub unsafe extern "C" fn game_appeallw(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        crate::TEMPLATE.article("template_fireball").spawn(&*agent);
    }
}
